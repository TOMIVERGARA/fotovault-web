		...
		{#each routes as { path, name, icon: Icon }}
			<Tooltip.Root openDelay={200}>
				<Tooltip.Trigger asChild let:builder>
					<a
						href={path}
						class="flex h-9 w-9 items-center justify-center rounded-lg transition-colors {$page.url.pathname.startsWith(
							path
						)
							? 'text-primary'
							: 'text-muted-foreground hover:text-foreground'} md:h-8 md:w-8"
						use:builder.action
						{...builder}
					>
						<Icon class="h-5 w-5" />
